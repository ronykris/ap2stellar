/**
 * Simple AI Agent Integration Example (TypeScript)
...
class AIAgentPaymentService {
  ...
  constructor(config: typeof AGENT_CONFIG) {
  ...
  async getQuote(sourceCurrency: string, destinationCurrency: string, amount: string): Promise<Quote> {
    ...
    try {
      ...
      console.error('❌ Quote error:', (error as Error).message);
      throw error;
    }
  }
   ...
   * Send a payment
   */
  async sendPayment(paymentDetails: PaymentDetails): Promise<PaymentResult> {
   ...
   */
  async checkPaymentStatus(intentId: string): Promise<PaymentResponse | null> {
   ...
   */
  async evaluatePayment(paymentRequest: PaymentDetails): Promise<PaymentEvaluation> {
 ...
 */
async function demonstrateAIAgentIntegration(): Promise<void> {
  console.log('═══════════════════════════════════════════════════════');
  ...
  const paymentRequest: PaymentDetails = {
    amount: '25.00',
    currency: 'XLM',
...
export { AIAgentPaymentService, PaymentDetails, PaymentResult, Quote };
